...
void CProgressWnd::CommonConstruct()
{
    ...
    m_strTitle       = _T(Utils::GetString(IDS_PROGRESS));
    m_strCancelLabel = _T(Utils::GetString(IDS_CANCEL_LABEL));
    m_bCancelled     = FALSE;
    m_bModal         = FALSE;
...
//        CenterWindow();
	...
	// Display window in upper right corner of screen (RenLib update)
	...
	rect.left   = desktopRect.right - rect.Width();
	rect.bottom = desktopRect.top   + rect.Height();
